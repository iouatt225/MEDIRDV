...
export default function PatientTeleconsultationPage({ params }: { params: Promise<{ appointmentId: string }> }) {
  ...
  const [chatHistory, setChatHistory] = useState<Array<{ sender: string; text: string }>>([
    { sender: 'Système', text: 'Bienvenue dans votre salon de téléconsultation sécurisé.' },
  ]);
    ...
    try {
      ...
      if (containerRef.current) {
        ...
        frame.on('left-meeting', () => {
          ...
          window.location.href = '/mes-rendez-vous';
        });
  ...
  const handleStartSimulation = async () => {
    ...
    // Attempt local webcam access for simulation
    try {
      ...
      console.warn('Webcam permission denied or unavailable, using mock canvas instead:', e);
    }
  };
  ...
  // Send simulation chat message
  const handleSendChat = (e: React.FormEvent) => {
    e.preventDefault();
    if (!chatMessage.trim()) return;

    setChatHistory((prev) => [...prev, { sender: 'Vous', text: chatMessage }]);
    setChatMessage('');
    ...
    setTimeout(() => {
      setChatHistory((prev) => [
        ...prev,
        { sender: 'Dr. Spécialiste', text: 'Bonjour, je prends note de votre message.' },
      ]);
    }, 2000);
  };
        ...
        <div className="max-w-[1200px] w-full mx-auto px-4 flex-1 flex flex-col">
          ...
          {!joined && (
            <div className="max-w-[600px] mx-auto w-full space-y-6 my-auto">
              <Link href="/mes-rendez-vous" className="inline-flex items-center gap-2 text-primary hover:text-accent font-semibold transition-colors">
              ...
              <Card hoverable={false} className="p-8 bg-white border border-divider text-center space-y-6">
                ...
                <div>
                  <span className="text-xs font-bold text-accent uppercase">Téléconsultation Médicale</span>
                  <h1 className="text-2xl font-bold text-primary mt-1">
                    Votre rendez-vous avec le médecin
                  </h1>
                  <p className="text-sm text-text mt-2">
                    {appointment ? `Prévu le ${new Date(appointment.slot_start).toLocaleDateString('fr-FR')} à ${new Date(appointment.slot_start).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}` : ''}
                  </p>
                </div>

                <div className="p-4 rounded bg-secondary/50 border border-divider text-xs text-text leading-relaxed">
                  Veuillez vous assurer d&apos;avoir une bonne connexion internet et d&apos;autoriser l&apos;accès à votre caméra et à votre micro lors de la connexion.
                </div>

                <div className="space-y-3">
                  <Button fullWidth onClick={handleJoinCall}>
                    Rejoindre la visioconférence (Daily.co)
                  </Button>
                  ...
                  {tokenError && (
                    <div className="space-y-3">
                      <div className="p-3 text-xs rounded bg-error/10 border border-error text-error text-center flex items-center gap-2 justify-center">
                        ...
                        Accès restreint. Le salon n&apos;est ouvert qu&apos;aux heures de rendez-vous (±15 min).
                      </div>
                  ...
                  <div className="w-full h-full relative flex flex-col justify-between p-4">
                    ...
                    <div className="absolute inset-0 bg-slate-900 flex items-center justify-center">
                      <div className="text-center text-white/50 space-y-3">
                        ...
                        <p className="text-sm font-semibold">Médecin connecté (Simulation)</p>
                      </div>
                    </div>

                    {/* Simulated Patient (Local) Video PIP */}
              ...
              {chatOpen && (
                <div className="w-full lg:w-80 bg-white rounded-xl border border-divider flex flex-col shadow-xs overflow-hidden">
                  <div className="p-4 border-b border-divider bg-secondary/30 flex items-center justify-between">
                    ...
                    <Badge variant="info">Sécurisé</Badge>
                  </div>
